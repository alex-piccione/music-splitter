...
## 2026-09-22
- Feature 10: each split part now carries a provenance COMM tag (language `eng`, description `Splitter provenance`) whose text is defined in `ui-text/english.yml`; not duplicated if the source already has one. Added `pyyaml` dependency; fixed invalid YAML block scalar in `english.yml`.
