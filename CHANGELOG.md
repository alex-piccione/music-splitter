...
## 2026-09-22
- Feature 7: the internal in-memory log is now a real log file (`music_splitter.log`, appended, timestamped `[YYYY-MM-DDTHH:MM:SS] LEVEL message`). Splitter warnings (provenance config fallbacks, metadata copy failures) are written to the same file instead of `print()`. A **Show log…** button appears below SPLIT whenever a split operation fails; clicking it opens a read-only viewer with the current log contents. Feature 11: split parts are now named `part_01.mp3` / `<source_stem>_part_01.mp3` instead of `01.mp3` / `<source_stem>_01.mp3`; radio button examples updated accordingly.
